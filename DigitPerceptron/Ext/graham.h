...
{
public :
	CGrahamScan(size_t n):N(n){};

    CGrahamScan( size_t n, int xmin, int xmax, int ymin, int ymax )
        : N( n )
        , x_range( xmin, xmax )
        , y_range( ymin, ymax )
    {
	...
	{
		raw_points.push_back( std::make_pair( x, y ) );
//		std::cout << raw_points.back() << " ";
	}

	void CreateHull(DPTLIST &ptlist)
	{
		partition_points();
//		log_partitioned_points( std::cout );
		build_hull();
		for ( size_t i = 0 ; i < lower_hull.size() ; i++ )
		{
			ptlist.push_back(CDPoint(lower_hull[i].first, lower_hull[i].second));
//            std::cout << lower_hull[ i ] << " ";
		}
        for ( std::vector< std::pair<double,double> >::reverse_iterator ii = upper_hull.rbegin() + 1 ;
              ii != upper_hull.rend();
              ii ++ )
		{
			std::pair<double,double> pt = *ii;
			ptlist.push_back(CDPoint(pt.first, pt.second));
//			std::cout << *ii << " ";
		}
	}
...
//                    plot_hull( f, "backtracking" );
    ...
    {
        return (int)( (p0.first - p1.first ) * (p2.second - p1.second ) )
    ...
    void log_partitioned_points( std::ostream &f )
    {
        ...
        f << "Partitioned set:\n"
          ...
          << "Right : " << right << "\n"
          << "Lower partition: ";
        for ( i = 0 ; i < lower_partition_points.size() ; i++ )
